...
class SemanticClassEditorDialog(QDialog):
    def __init__(self, parent=None, sem_class: SemanticClass = None):
        super().__init__(parent)
...
class VTKPointCloudViewer(QMainWindow):
    def __init__(self):
        ...
        # Updated tool list: wand tool now replaces pipette tool.
    ...
    def update_segmentation_browser(self):
        ...
        for seg in self.segmentations:
            ...
            if stat.get("instance_statistic") is not None:
                inst_stat = stat["instance_statistic"]
                inst_item = QTreeWidgetItem([
                    "Instance labels", 
                    str(inst_stat["number_of_instances"]), 
                    str(inst_stat["amount_of_instance_points"][0]),
                    f"{inst_stat['amount_of_instance_points'][1]*100:.1f}%"
                ])
            else:
    ...
    def on_seg_tree_selection_changed(self):
        ...
        elif item.text(0) == "Instance labels":
            parent = item.parent()
            if parent is not None:
                self.active_segmentation = parent.data(0, Qt.ItemDataRole.UserRole)
    ...
    def wand_pick(self):
        # Use point picker to get clicked point index.
    ...
    def lasso_segmentation(self):
        ...
        # Do not change the current coloring of the point cloud.
        ...
        # Exit the lasso tool if the control panel is closed (via the system UI 'X')
